  ...
  const checkAuth = async () => {
    try {
      ...
      if (response.logged_in) {
        ...
        try {
          const admin = await adminApi.getAdminInfo();
          setAdminInfo(admin);
        } catch (adminInfoError) {
          console.error('Failed to fetch admin info:', adminInfoError);
        }
        loadEmployees();
      } else {
  ...
  const handleSendMessage = async (context: string) => {
    ...
    try {
      await adminApi.createMessage({
        ...
        message: newMessage,
        sender: 'admin',
        sender_name: adminInfo?.name || 'BrainHR Admin',
        sender_id: adminInfo?.id || 1,
        sender_type: 'admin',
    ...
    <div className="min-h-screen bg-gray-50 pt-20 pb-20">
      <div className="container mx-auto px-4">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center gap-4">
            <NotificationBell userType="admin" onRefresh={() => selectedEmployee && loadEmployeeData(selectedEmployee.id)} />
...
function MessagesTab({ empId, data, messageFilter, setMessageFilter, employees, managers, onOpenSendMessage }: any) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2 border-b pb-4 items-center justify-between flex-wrap">
        <div className="flex gap-2">
          <Button
